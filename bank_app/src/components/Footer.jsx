...
const Footer = () => (
      <section className={`${styles.paddingY} ${styles.flexCenter} flex-col`}>
        ...
        <div className='w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-top-[#3f3r45]'>
          <p className='font-poppins font-normal text-[18px] leading-[27px] text-white'> 
                        2022 HooBank. All Rights Reserved.
          </p>
          <div className='flex flex-row md:mt-0 mt-6'>
            {socialMedia.map((media, index) => (
               <img src={media.icon} key={media.id} alt={media.id} className={`w-[21px] h-[21px] object-contain cursor-pointer ${index !== socialMedia.length - 1 ? 'mr-6' : 'mr-0' }`}/>
            ))} 
